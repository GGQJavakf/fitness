ALTER TABLE workout_set
    ADD COLUMN safety_flag VARCHAR(32) NULL AFTER server_revision,
    ADD CONSTRAINT ck_workout_set_safety_flag CHECK (
        safety_flag IS NULL OR safety_flag IN (
            'PAIN', 'INJURY', 'CHEST_DISCOMFORT', 'DIZZINESS', 'SEVERE_UNWELL'
        )
    );

-- Corrections retain immutable identity while allowing the audited payload fields,
-- including safety_flag and its digest/revision metadata, to advance together.
