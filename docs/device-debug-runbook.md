...
若应用组合根或页面渲染在网络请求前同步失败，全局错误边界必须显示“页面加载失败”和“返回首页”，不得停留在纯白页。返回首页只重建运行时，不清除本地训练记录；若恢复页本身仍反复出现，应停止验收并保留设备、微信基础库和控制台错误证据。
...
1. 冷启动后必须出现首页、明确配置错误页或明确恢复页之一，不允许纯白页；连续执行清缓存启动、保留缓存启动和杀进程后启动。
2. 登录并完成建档、器械选择和计划确认。
3. 开始训练，执行通用热身和递增热身组。
4. 完成正式组并验证休息计时在前后台切换后按结束时间恢复。
5. 分别执行锁屏、杀进程、弱网、飞行模式、响应丢失和连续点击场景。
6. 恢复网络后确认幂等同步、显式冲突处理和已完成组不丢失。
7. 完成训练，检查历史、趋势、进阶建议和 AI 模板降级总结。

每次执行应在上级产品文档的 `doc/verification/M2_微信真机恢复矩阵.md` 记录设备、系统、微信、基础库版本及结果。自动化构建、模拟器运行或错误恢复页只能证明包可构建、故障可诊断，不能替代 Android 真机证据；没有这些证据时，不得宣称真机门已通过。
